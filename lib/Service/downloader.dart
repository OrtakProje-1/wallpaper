...
import 'package:wallpaper/Class/downloadPath.dart';
import 'package:wallpaper/Class/permissionsClass.dart';
import 'package:wallpaper/Models/downloadTask.dart';
import 'package:wallpaper_manager_flutter/wallpaper_manager_flutter.dart';

class Downloader {
  DownloadPath downloadPath = DownloadPath();
  CacheManager customCachManager = CacheManager(Config("asdasd"));
  ...
  Future<bool> chachDownloadandSetWallpaper(String? url, int? screen) async {
    ...
    try {
      ...
      WallpaperManagerFlutter().setwallpaperfromFile(info.file.path, screen);
      return true;
    } catch (e) {
      return false;
    }
  ...
  saveGalery(String _localPath, Task task) async {
    bool sonuc = await (GallerySaver.saveImage(
        ...
        albumName: "H~Wallpaper") as FutureOr<bool>);
    if (sonuc) print("Galeriye kayıt başarılı");
  }
